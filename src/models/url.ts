...
// Ensure indexes are only created once
if (
  !URLSchema.indexes().some((index) => index[0].hasOwnProperty("originalUrl"))
) {
  URLSchema.index({ originalUrl: 1 });
}
if (
  !URLSchema.indexes().some((index) => index[0].hasOwnProperty("shortenUrl"))
) {
